...
This section refines the analysis into the subset of requirements that the implemented project actually targets. The resulting system is a single-master MapReduce library that executes one job at a time, delegates input access to a serializable \texttt{DataProviderClient}, tolerates worker failures, and combines in-memory coordination state with local ephemeral storage on the master and workers.
...
\begin{enumerate}[label=\textbf{IR-\arabic*}]
    ...
    \item \textbf{Docker Support}
          \begin{itemize}
              \item The project shall provide a Docker-based deployment path for reproducible experiments.
              \item \textit{Acceptance Criterion:} Master, worker, and demo jobs can be launched with the supplied Docker scripts and compose configuration.
          \end{itemize}
\end{enumerate}
