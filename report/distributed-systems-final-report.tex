...
\begin{enumerate}[label=\textbf{NFR-\arabic*}]
    ...
    \item \textbf{Reproducible Deployment}
          \begin{itemize}
              \item The system shall be runnable both as ordinary Java processes and through published Docker images.
              \item \textit{Acceptance Criterion:} Master, worker, and demo job artifacts can be executed from released fat JARs or from public Docker images without changing the public programming model.
          \end{itemize}
...
\begin{enumerate}[label=\textbf{IR-\arabic*}]
    ...
    \item \textbf{Docker Support}
          \begin{itemize}
              \item The project shall provide Docker images for the master and worker runtime and shall interoperate with the public Docker images of the demo job.
              \item \textit{Acceptance Criterion:} A user can start a complete cluster using the published images \texttt{leo02n/jmr-master}, \texttt{leo02n/jmr-worker}, and \texttt{leo02n/jmr-wc}.
          \end{itemize}
\end{enumerate}
...
The deployment topology presented as a reference use case is shown in \cref{fig:deployment-topology}. In the packaged Docker setup, the master and workers are individual containers attached to the same virtual network, while the submitter container from the word-count example joins that network for the duration of the job and starts the local data provider inside the submitter process.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{diagrams/deployment_topology.png}
    \caption{Deployment topology used by the packaged Docker demo}
...
Validation is organized on three levels: Java unit tests for deterministic low-level behaviour, internal Python-driven integration tests for cluster scenarios, and manual acceptance checks for the dashboards and operator workflow.
...
The repository also contains top-level Python integration tests that orchestrate a real Docker cluster. These tests are internal development and validation tooling: they are useful because many requirements concern interactions among components rather than isolated methods, but they are not part of the public deployment path expected from end users.
...
The public deployment story is intentionally limited to two paths: released fat JARs for users who want to run ordinary Java processes, and published Docker images for users who want a ready-made local cluster. The Python tests and convenience shell scripts used during development are internal validation tooling; they are intended neither to be run nor guaranteed to work with the delivered system.

\paragraph{Prerequisites.}
The main software prerequisites are:
%
\begin{itemize}
    \item \textbf{JDK 21+} for the fat-JAR deployment;
    \item \textbf{Maven} only if the artifacts are rebuilt from source rather than downloaded from the release pages.
    \item \textbf{Docker Engine or Docker Desktop with Docker Compose} for the containerized demo;
\end{itemize}

\paragraph{Configuration.}
No mandatory environment variables are required by the core runtime. In the Java-process deployment, configuration is command-line based: each worker receives its id, port, and storage directory, while the master receives its port, storage directory, and optionally the static worker endpoints. In the Docker Compose deployment, the integration repository provides the compose files and a small \texttt{submitter.env} file for the demo job parameters such as input directory, output directory, result filename, and master endpoint.

\paragraph{Fat-JAR deployment.}
The master and worker artifacts are published as runnable fat JARs. After downloading or building them, a minimal local cluster with automatic JmDNS discovery can be launched as follows:
%
\begin{verbatim}
java -Djava.net.preferIPv4Stack=true \
  -jar jmr-worker.jar -p 10001 -w 1 -sd worker-data-1

java -Djava.net.preferIPv4Stack=true \
  -jar jmr-worker.jar -p 10002 -w 2 -sd worker-data-2

java -Djava.net.preferIPv4Stack=true \
  -jar jmr-master.jar -p 10000 -sd master-data
\end{verbatim}
...
\begin{verbatim}
java -Djava.net.preferIPv4Stack=true \
  -cp jmr-worker.jar it.jmr.worker.StaticWorkerLauncher \
...
java -Djava.net.preferIPv4Stack=true \
  -cp jmr-master.jar it.jmr.master.StaticMasterLauncher \
  -p 10000 -sd master-data \
  --worker 1:127.0.0.1:10001 \
  --worker 2:127.0.0.1:10002
\end{verbatim}

\paragraph{Docker Compose deployment with public images.}
The packaged Docker demo is maintained in the separate integration repository \url{https://github.com/leorm2002/jmr-integration}. It consumes the public images \texttt{leo02n/jmr-master}, \texttt{leo02n/jmr-worker}, and \texttt{leo02n/jmr-wc}; no local image build is required. A complete run is:
%
\begin{verbatim}
git clone https://github.com/leorm2002/jmr-integration
cd jmr-integration
docker compose -f docker-compose.yml up -d
docker compose --env-file submitter.env -p submit-wc \
  -f docker-compose.submitter.yml up \
  --abort-on-container-exit --exit-code-from submitter submitter
docker compose --env-file submitter.env -p submit-wc \
  -f docker-compose.submitter.yml down -v --remove-orphans
docker compose -f docker-compose.yml down -v --remove-orphans
\end{verbatim}

Expected outcome: the master is reachable on \texttt{localhost:50050}, the master dashboard is reachable on \texttt{http://localhost:51050}, worker dashboards are exposed on ports \texttt{51051}--\texttt{51053}, and the word-count result is written to the output path configured by \texttt{submitter.env}.

\paragraph{How containerization was engineered.}
Containerization follows role-specific images rather than a single multi-purpose image. The main repository builds and publishes the master and worker runtime images through dedicated GitHub Actions workflows. The separate word-count repository builds and publishes the demo submitter images. The integration repository then composes these public images into a fixed three-worker deployment with persistent named volumes and explicit worker definitions. This separation keeps runtime, example job, and deployment concerns independent.
...
For the packaged demo with published Docker images:
%
\begin{enumerate}
    \item clone the integration repository \url{https://github.com/leorm2002/jmr-integration};
    \item start the cluster with \texttt{docker compose -f docker-compose.yml up -d};
    \item submit the word-count job with the \texttt{docker compose} command documented in the integration repository;
    ...
    \item stop the cluster with \texttt{docker compose -f docker-compose.yml down -v --remove-orphans}.
\end{enumerate}

Expected outcome: the job progresses through MAP and REDUCE, produces a CSV output in the output directory configured by \texttt{submitter.env}, and leaves an event trail visible in both master and worker dashboards.
...
The strongest aspect of the project is architectural coherence. The resulting library is small enough to understand, but still demonstrates the main concerns of a distributed batch system: job submission, code distribution, remote execution, shuffle, observability, and recovery after worker loss. Another strong point is the operational tooling: dashboards, released fat JARs, and published Docker images make the system easier to inspect and demonstrate.

\paragraph{Weaknesses of the product.}
The main weaknesses come from the intentionally reduced scope. The master is a single point of failure, security is minimal, scheduling is simple, and there is no support for restarting a failed cluster. User-provided code is trusted and loaded without sandboxing, so stronger isolation would be required in a production-grade evolution.
